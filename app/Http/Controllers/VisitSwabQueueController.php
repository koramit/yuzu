...
class VisitSwabQueueController extends Controller
{
    ...
    public function store(Visit $visit)
    {
        ...
        $errors = $manager->validateSwabAppointment($data);

        $manager->saveVisit($visit, $data);
        ...
        // status
        $visit->status = 'swab';
        $visit->submitter_id = Auth::id();
        $visit->forceFill([
            'form->md' => $manager->getIdStaff($data['md_name']),
        ]);
        $visit->save();

        return Redirect::route('visits.swab-queue');
    }
}
